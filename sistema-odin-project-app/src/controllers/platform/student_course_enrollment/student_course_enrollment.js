...
// Get all the courses enrolled for the student (payment)
export async function getStudentCourseEnrollmentsPaidSingleUser(platform_user_id) {
  try {
    const { data, error } = await supabase
      .from("student_course_enrollments")
      .select("*")
      .eq("platform_user_id", platform_user_id)
      .eq("platform_state_id", 3);
