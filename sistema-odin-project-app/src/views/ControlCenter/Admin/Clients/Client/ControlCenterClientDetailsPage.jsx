...
import {
  deleteClient,
  getClient,
} from "@/src/controllers/control_center/cc_client/cc_client";
...
export default function ControlCenterClientDetailsPage({
  controlCenterClientId,
}) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const fetchedClient = await getClient(controlCenterClientId);
    ...
    fetchData();
  }, [controlCenterClientId, showNotification, userControlCenter]);
  ...
  const confirmDelete = async () => {
    try {
      await deleteClient(controlCenterClientId);
          ...
          {(userControlCenter?.cc_user_role_id === 2 ||
            ...
            <>
              <Button
                ...
                route={`/control_center/admin/clients/${controlCenterClientId}/edit`}
                isAnimated={false}
                icon={<FiEdit size={24} />}
                title="Editar"
              />
