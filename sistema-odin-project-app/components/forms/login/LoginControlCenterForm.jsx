...
export default function LoginControlCenterForm({ onCloseModal }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await editControlCenterUserStatus(foundUser.id, true);

      // Realizar el inicio de sesión con axios
      const loginResponse = await axios.post(
        `/api/auth/login_control_center`,
        foundUser
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-100 flex items-center justify-center z-50">
      ...
      {showNotification && (
        <NotificationComponent
          ...
          message="Continua gestionando tus procesos en nuestro centro de control."
          icon="/logo.png"
        />
      )}
    </div>
